...
var BaseLiveTourCalcViewModel = /** @class */ (function (_super) {
    ...
    // スキル効果適用可能チェック
    BaseLiveTourCalcViewModel.prototype.check_apply_skill = function (idol, skill) {
        var result = false;
        var type = parseInt(idol.type);
        // スキルが効果適用可能かチェック
        if (skill.target_unit == SKILL_TARGET_UNIT.OWN) {
...
var ViewModel = /** @class */ (function (_super) {
    ...
    function ViewModel() {
        var _this = _super.call(this) || this;
        // 入力値
        _this.calc_type = CALCULATION_TYPE.ROYAL.toString();
        _this.battle_point = "2";
        // セーブデータ関係
        _this.save_data_key = "imas_cg_live_royal_calc";
        _this.init_list();
        ko.track(_this);
        return _this;
    }
    ...
    ViewModel.prototype.is_guest_live = function () { return (parseInt(this.calc_type) == CALCULATION_TYPE.ROYAL_GUEST); };
    // 発揮値計算
    ...
    // スキル効果適用可能チェック
    ViewModel.prototype.check_apply_skill = function (idol, skill) {
        if (this.is_guest_live()) {
            return _super.prototype.check_apply_skill.call(this, idol, skill);
        }
        var result = false;
        var type = parseInt(idol.type);
        // スキルが効果適用可能かチェック
